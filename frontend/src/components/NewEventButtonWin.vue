...
methods: {
    ...
    async showConfirmDialog() {
            if (this.dialogRef) {
                ...
                try {
        const response = await fetch(`https://observational.website/polls/confirm/${parseInt(this.$route.params.id)}`, {
    ...
    async submitEvent() {
      ...
      try {
        const response = await fetch('https://observational.website/events/new', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
